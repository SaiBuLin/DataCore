...
public class DeleteFormSQLParser extends SQLParser implements SQLParserAble
{
		...
		try{
			...
			if( config.getFormPart() != null ){
				...
				if( formparstAble != null ){
					...
					logger.error("DeleteForm 的FormPart创建PraserAble 失败。");
				}
			}else{
				logger.error("DeleteForm 的FormPart没有配置。");
			}
	...
	public CommandForm parserCommand(VOForm form) throws DataServiceException{
		...
		try{
			...
			logger.debug("DeleteFormSQLParser开始执行parserCommand。");	
			...
			if( config == null){			
				throw new Exception("DeleteFormSQLParser错误：parserCommand, queryDefineElement构型为SelectForm错误。" );
			}
			
			if( config.getFormPart() == null){
				throw new Exception("DeleteFormSQLParser错误：parserCommand, FormPart没有配置。" );
			}
			
			if( config.getDataSet() == null){
				throw new Exception("DeleteFormSQLParser错误：parserCommand, DataSet没有配置。" );
			}
			
			if( config.getActionTools() == null ){
				throw new Exception("DeleteFormSQLParser错误：parserCommand, ActionTools没有配置。" );
			}
			
			SQLParserAble partAble = SQLParserFactory.getQueryDefineInterpreter(config.getFormPart() );
			
			if( partAble == null){
				throw new Exception("DeleteFormSQLParser错误：parserCommand, FormPart不能正确对应正确的PraserTools。"  );
			}
			...
			if( actionToolsAble == null){
				throw new Exception("DeleteFormSQLParser错误：parserCommand, ActionTools不能正确对应正确的PraserTools。"  );
			}
			...
			logger.debug("DeleteFormSQLParser执行parserCommand完毕。");	
			...
			if( e != null ){
				String execeptionClassName =  e.getClass().toString();
				if( !UtilTools.isNull(execeptionClassName)){
					logger.error("DeleteFormSQLParser::parserCommand 抛出异常， 异常类：" + execeptionClassName);
				}
				logger.error("DeleteFormSQLParser::parserCommand 抛出异常， 异常信息：" + e.getMessage());
				String localMessage = e.getLocalizedMessage();
				if( !UtilTools.isNull(localMessage)){
					logger.error("DeleteFormSQLParser::parserCommand 抛出异常， 异常信息：" + localMessage);
				}
			}
		}
	...
	public NetDataSet executeVOAction(VOForm form, String formcode) throws DataServiceException{
		...
		/* VO Schema  */
		try{
			logger.debug("DeleteFormSQLParser 执行executeVOAction。");
			...
			if(UtilTools.isNull( config.getCode() )){
				logger.error("DeleteForm 的Code没有配置。");
				return result;
			}
			...
			if( actionTools == null ){
				logger.error("DeleteForm配置ActionTools创建SQLParserAble执行parseFormActionTools错误。");
				return result;
			}
			...
			logger.debug("DeleteFormSQLParser 执行executeVOAction完毕。");
			return result;
		}catch(Exception e){
			if( e != null ){
				String execeptionClassName = e.getClass().toString();
				if( !UtilTools.isNull(execeptionClassName)){
					logger.error("DeleteFormSQLParser::executeVOAction 抛出异常， 异常类：" + execeptionClassName);
				}
				logger.error("DeleteFormSQLParser::executeVOAction 抛出异常， 异常信息：" + e.getMessage());
				String localMessage = e.getLocalizedMessage();
				if( !UtilTools.isNull(localMessage)){
					logger.error("DeleteFormSQLParser::executeVOAction 抛出异常， 异常信息：" + localMessage);
				}
			}
		}
		return null;
