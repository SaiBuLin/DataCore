...
public class VISchemaSQLParser extends SQLParser implements SQLParserAble
{
	...
	public void loadSchemas( Map<String, Schema> schema_map, Map<String, Object> properties ) throws Exception{
		...
		try{
			...
			/* 强制转换 */
			logger.debug("loadSchemas 执行：将queryDefineElement强制转换成VISchema.");
			...
			if( e != null ){
				...
				if( !UtilTools.isNull(execeptionClassName)){
					logger.error("VISchemaSQLParser::loadSchemas 抛出异常， 异常类：" + execeptionClassName);
				}
				logger.error("VISchemaSQLParser::loadSchemas 抛出异常， 异常信息：" + e.getMessage());
				String localMessage = e.getLocalizedMessage();
				if( !UtilTools.isNull(localMessage)){
					logger.error("VISchemaSQLParser::loadSchemas 抛出异常， 异常信息：" + localMessage);
				}
			}
		}
	}
